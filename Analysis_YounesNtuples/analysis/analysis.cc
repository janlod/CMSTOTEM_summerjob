...
void analysis(){
    std::string dir_path = "../YounesNtuples";
    std::vector<std::string> filenames;

    for (const auto& entry : fs::directory_iterator(dir_path)) {
        if (entry.is_regular_file()) {
            filenames.push_back(entry.path().string());  // Full path
            // or use: entry.path().filename().string() for just the filename
        }
    }
    ...
    TFile *totem20 = new TFile("../YounesNtuples/TOTEM20.root", "read");
    TTree *tree = (TTree*)totem20->Get("tree");
    ...
    Float_t* trk_p = new Float_t[10000];
    Float_t* trk_pt = new Float_t[10000];
    Float_t* trk_eta = new Float_t[10000];
    Float_t* trk_phi = new Float_t[10000];
    Int_t*   trk_q = new Int_t[10000];
    Int_t*   trk_isPi = new Int_t[10000];
    Int_t*   trk_isK = new Int_t[10000];
    Int_t*   trk_isP = new Int_t[10000];
    Float_t* trk_dz = new Float_t[10000];
    Float_t* trk_dxy = new Float_t[10000];
    Float_t* trk_dedx = new Float_t[10000];
    Float_t* trk_dedxerr = new Float_t[10000];
    Int_t*   trk_nSaturMeasure = new Int_t[10000];
    Int_t*   trk_nMeasure = new Int_t[10000];
    Int_t*   trk_nMeasureLayer = new Int_t[10000];
    Float_t* trk_dxyerr = new Float_t[10000];
    Float_t* trk_dzerr = new Float_t[10000];
    Float_t* trk_pterr = new Float_t[10000];
